...
Some commands (`projects`, `api-keys`) manage org-wide resources and require an org-level API key. Set `LANGWATCH_ORG_API_KEY` in your `.env`:

```bash
echo 'LANGWATCH_ORG_API_KEY=sk-lw-...' >> .env
```

The CLI falls back gracefully: `LANGWATCH_ORG_API_KEY` → `LANGWATCH_API_KEY` (with a warning) → error with setup instructions. If only a project-scoped key is available, the CLI warns that the operation may fail due to insufficient permissions.
...
These commands require an org-level API key (`LANGWATCH_ORG_API_KEY`). See [Organization-level auth](#organization-level-auth).
