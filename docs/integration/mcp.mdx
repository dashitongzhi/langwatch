...
<Info>
Organization management tools require an org-level API key. Pass it via `LANGWATCH_ORG_API_KEY` in your MCP server configuration. If only a project-scoped key is available, these tools will return a clear error message.
</Info>
